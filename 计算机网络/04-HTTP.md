...
### [HTTP/2.0](https://zhuanlan.zhihu.com/p/26559480)

1. 流传输（二进制帧）
   1. HTTP/1.x请求和响应都是由起始行，首部和正文组成，各部分用换行分隔符，是文本格式
   2. HTTP/2.0将请求和响应数据分割成更小的帧，采用二进制编码
2. 多路复用
   1. 一个域名只用一个TCP长连接进行数据传输，所有通信都在一个连接上完成
   2. 单个连接可以承载任意数量的双向数据流
   3. 数据流以消息的形式发送，而消息又由一个或多个帧组成，乱序的帧都会根据帧首部的流标识重新组装
3. 头部压缩
...
2. 但是并发请求必须使用多个TCP连接
3. 管道机制
   1. HTTP维护一个队列，可以同时发送多个请求，直到请求处理完成后关闭连接
   2. 但是同一时刻只能处理一个请求，队首处理时间过长，会导致其他请求一直等待，造成队头堵塞

### keep-alive开启/关闭

1. Connection: Keep-Alive （HTTP/1.1后默认开启）
2. Connection: close
