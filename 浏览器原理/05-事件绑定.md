...
相对与mouseover/mouseout，但是当元素没有子元素时作用是一致的，但是他们俩会触发冒泡
...
1. 事件捕获阶段
2. 事件处理阶段（处理时机按照addEventListener的第三个参数决定，默认冒泡阶段触发）
3. 事件冒泡阶段
...
2. 计算多：mousemove、mouseout可以冒泡，但是触发频繁，计算过多
