...
1. 如果多个入口引入了lodash，那么lodash将会同时打包到所有js模块中
2. 不灵活，不能根据核心应用程序的逻辑来动态分割代码
3. plugins中增加多个HtmlWebpackPlugin的类用作html多入口，同时设置chunk指定引入js，否则会都引入

## 提取公共代码

1. SPA（单页面应用），打包完成时，只有一个bundle.js
2. 代码体积大，不利于下载
...
1. SplitChunksPlugin，webpack自带
2. 配置optimization.splitChunks就可以开启
...
1. 代码使用时增加/* webpackChunkName: "show" */表示当前为动态导入

2. webpack在遇到/* webpackChunkName: "show" */会进行以下处理
   1. 以./show.js重新创建一个Chunk
   2. 当代码执行到import才会加载文件
   3. import返回一个Promise，then后才能执行
3. 同时指定output设置对应文件输出文件名称
...
#### prefetch：在浏览器空闲的时候加载未来可能会用到的资源
...
#### preload：先加载本页面需要用到的关键资源，不会等待浏览器空闲
